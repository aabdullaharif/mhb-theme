...
`;
  ...
  var ProductMeta = class extends HTMLElement {
    ...
    _updateLabels(variant) {
      let productLabelList = document.querySelector("[data-product-label-list]");
      if (!productLabelList) {
        return;
      }
      if (!variant) {
    ...
    _updatePrices(variant) {
      let productPrices = document.querySelector("[data-product-price-list]"), currencyFormat = window.themeVariables.settings.currencyCodeEnabled ? window.themeVariables.settings.moneyWithCurrencyFormat : window.themeVariables.settings.moneyFormat;
      ...
      if (!variant) {
        ...
        if (variant["compare_at_price"] > variant["price"]) {
          ...
          productPrices.innerHTML += `<span class="price price--compare"><span class="visually-hidden">${window.themeVariables.strings.productRegularPrice}</span>${formatMoney(variant["compare_at_price"], currencyFormat)}<span class="compare-at-new"> NEW</span></span>`;
        } else {
